...
describe OmniAuth::Strategies::Valid do
  ...
  describe "client options" do
    ...
    it "has the correct site" do
      expect(subject.client.site).to eq("https://valid-pre.aoc.cat")
    end

    it "has the correct authorize url" do
      expect(subject.client.options[:authorize_url]).to eq("https://valid-pre.aoc.cat/o/oauth2/auth")
    end

    it "has the correct token url" do
      expect(subject.client.options[:token_url]).to eq("https://valid-pre.aoc.cat/o/oauth2/token")
    end

    it "has correct authorize params" do
      # https://valid-pre.aoc.cat/o/oauth2/auth?client_id=xxxxxx&redirect_uri=http%3A%2F%2Flocalhost%3A3000%2Fusers%2Fauth%2Fidcat_mobil%2Fcallback&response_type=code&state=2ec1a9a9db94fbce7f19ee30d682ab75f97f4fd67848773c&approval_prompt=auto&scope=autenticacio_usuari&response_type=code
